  ...
  return new Promise((resolve, reject) => {
    const worker = createWorker(() => {
      ...
      function calculateRelevance(result: any, query: string): number {
        ...
        if (result.tag_id?.toLowerCase().includes(queryLower)) score += 10;
        if (result.description?.toLowerCase().includes(queryLower)) score += 5;
        if (result.make?.toLowerCase().includes(queryLower)) score += 3;
        if (result.model?.toLowerCase().includes(queryLower)) score += 3;
