...
export function useSearchSuggestions(
  ...
  useEffect(() => {
    if (autoFetch && debouncedQuery.trim().length >= minQueryLength) {
  ...
  const allSuggestions = useMemo(() => {
    ...
    if (includeRecent && query.trim().length === 0) {
      ...
      const recentItems = recentSearches.slice(0, Math.max(5, maxSuggestions / 2)).map(search => ({
        label: search.query,
      ...
      const matchingRecent = recentSearches
        .filter(
          search =>
            search.query.toLowerCase().includes(query.toLowerCase()) && search.query !== query
        )
        .slice(0, 3)
        .map(search => ({
          label: search.query,
    ...
    return combined;
  }, [includeRecent, query, recentSearches, suggestions, maxSuggestions]);
