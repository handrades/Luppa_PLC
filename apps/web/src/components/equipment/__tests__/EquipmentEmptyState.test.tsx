...
describe('EquipmentEmptyState', () => {
  ...
  describe('rendering without filters', () => {
    it('should render empty state for no data', () => {
      renderWithTheme(<EquipmentEmptyState {...defaultProps} hasFilters={false} />);

      expect(screen.getByText('No equipment available')).toBeInTheDocument();
      expect(
        screen.getByText('Get started by adding your first piece of equipment to the system.')
      ).toBeInTheDocument();
    });
    ...
    it('should show getting started help', () => {
      ...
      expect(screen.getByText(/Add PLCs, robots, conveyors/)).toBeInTheDocument();
    });
  ...
  describe('rendering with filters', () => {
    it('should render empty state for no search results', () => {
      renderWithTheme(<EquipmentEmptyState {...defaultProps} hasFilters={true} />);

      expect(screen.getByText('No equipment found')).toBeInTheDocument();
      expect(
        screen.getByText('Try adjusting your search criteria or filters to find equipment.')
      ).toBeInTheDocument();
    });
    ...
    it('should show search tips', () => {
      ...
      expect(screen.getByText(/Search by equipment description/)).toBeInTheDocument();
    });
  ...
  describe('button interactions', () => {
    it('should call onClearFilters when clear filters button is clicked', () => {
      ...
      renderWithTheme(
        <EquipmentEmptyState {...defaultProps} hasFilters={true} onClearFilters={onClearFilters} />
      );
    ...
    it('should call onAddEquipment when add equipment button is clicked', () => {
      ...
      renderWithTheme(
        <EquipmentEmptyState {...defaultProps} hasFilters={false} onAddEquipment={onAddEquipment} />
      );
    ...
    it('should not render add equipment button when onAddEquipment is not provided', () => {
      renderWithTheme(
        <EquipmentEmptyState {...defaultProps} hasFilters={false} onAddEquipment={undefined} />
      );
  ...
  describe('icon rendering', () => {
    it('should render inventory icon when no filters', () => {
      ...
      const iconContainer = container.querySelector('[data-testid="Inventory2OutlinedIcon"]');
      expect(iconContainer || container.querySelector('svg')).toBeInTheDocument();
    });
    ...
    it('should render search off icon when filters are active', () => {
      ...
      // Check for the icon container
      const iconContainer = container.querySelector('[data-testid="SearchOffIcon"]');
      expect(iconContainer || container.querySelector('svg')).toBeInTheDocument();
    });
  });
});
