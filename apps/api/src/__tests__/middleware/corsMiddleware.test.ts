...
describe('CORS Middleware', () => {
  ...
  describe('development environment', () => {
    ...
    it('should allow requests from localhost:4173 (Vite preview server)', async () => {
      const response = await request(app).get('/test').set('Origin', 'http://localhost:4173');
  ...
  describe('production environment', () => {
    ...
    beforeAll(() => {
      ...
      const { corsMiddleware: prodCorsMiddleware } = require('../../middleware/corsMiddleware');
    ...
    it('should allow production domain', async () => {
      const response = await request(app).get('/test').set('Origin', 'https://inventory.local');

      expect(response.status).toBe(200);
      expect(response.headers['access-control-allow-origin']).toBe('https://inventory.local');
    });
