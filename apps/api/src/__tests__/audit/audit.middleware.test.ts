...
describe("Audit Context Middleware", () => {
  ...
  let mockResponse: Partial<
    Response & { _finishCallback?: () => Promise<void> }
  >;
  ...
  describe("Session Context Setting", () => {
    it("should set PostgreSQL session variables for authenticated user", async () => {
      ...
      expect(mockQueryRunner.query).toHaveBeenCalledWith(
        "SET app.client_ip = '10.0.0.1'",
      );
      expect(mockQueryRunner.query).toHaveBeenCalledWith(
        "SET app.user_agent = 'Test User Agent'",
      );
      expect(mockQueryRunner.query).toHaveBeenCalledWith(
        "SET app.session_id = 'test-session-id'",
      );
