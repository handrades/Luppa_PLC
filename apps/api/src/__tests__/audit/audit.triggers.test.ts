...
describe("Audit Triggers Integration Tests", () => {
  ...
  describe("Audit Trigger Function", () => {
    ...
    it("should create audit log for UPDATE operations with changed fields", async () => {
      ...
      await queryRunner.query(
        "UPDATE sites SET name = $1, updated_by = $2 WHERE id = $3",
        ["New Site Name", "test-user-id", "site-1"],
      );
    ...
    it("should handle missing session context with fallback", async () => {
      ...
      await queryRunner.query("INSERT INTO sites (name) VALUES ($1)", [
        "Untracked Site",
      ]);
    ...
    it("should capture all session context variables", async () => {
      ...
      await queryRunner.query(
        `SET app.current_user_id = '${sessionContext.user_id}'`,
      );
      await queryRunner.query(
        `SET app.client_ip = '${sessionContext.ip_address}'`,
      );
      await queryRunner.query(
        `SET app.user_agent = '${sessionContext.user_agent}'`,
      );
      await queryRunner.query(
        `SET app.session_id = '${sessionContext.session_id}'`,
      );
  ...
  describe("Trigger Performance", () => {
    it("should execute all batch operations successfully", async () => {
      ...
      const operations = Array.from({ length: 10 }, (_, i) =>
        queryRunner.query(
          "INSERT INTO sites (name, created_by, updated_by) VALUES ($1, $2, $3)",
          [`Site ${i}`, "test-user-id", "test-user-id"],
        ),
      );
    ...
    it("should handle concurrent operations correctly", async () => {
      ...
      const concurrentOps = Array.from({ length: 5 }, (_, i) =>
        queryRunner.query(
          "INSERT INTO sites (name, created_by, updated_by) VALUES ($1, $2, $3)",
          [`Concurrent Site ${i}`, `user-${i}`, `user-${i}`],
        ),
      );
  ...
  describe("Audit Log Immutability", () => {
    it("should prevent updates to audit logs", async () => {
      queryRunner.query = jest
        .fn()
        .mockRejectedValueOnce(new Error("Permission denied")); // UPDATE should fail

      await expect(
        queryRunner.query(
          "UPDATE audit_logs SET risk_level = $1 WHERE id = $2",
          ["LOW", "audit-1"],
        ),
    ...
    it("should prevent deletion of audit logs", async () => {
      queryRunner.query = jest
        .fn()
        .mockRejectedValueOnce(new Error("Permission denied")); // DELETE should fail
  ...
  describe("Index Performance", () => {
    it("should use correct SQL query with proper parameters for indexed lookup", async () => {
      queryRunner.query = jest
        .fn()
        .mockResolvedValueOnce([
          { id: "audit-1", table_name: "sites", record_id: "site-1" },
        ]);
    ...
    it("should efficiently query audit logs by user and timestamp", async () => {
      queryRunner.query = jest
        .fn()
        .mockResolvedValueOnce([
          { id: "audit-1", user_id: "test-user", timestamp: new Date() },
        ]);
