...
describe("AuditService", () => {
  ...
  beforeEach(() => {
    ...
    (auditService as { auditRepository: AuditRepository }).auditRepository = mockAuditRepository;
  });
  ...
  describe("getAuditLogs", () => {
    ...
    it("should handle repository errors", async () => {
      mockAuditRepository.findAuditLogs.mockRejectedValue(new Error("Database error"));

      await expect(auditService.getAuditLogs({})).rejects.toThrow("Failed to retrieve audit logs");
    });
  });
  ...
  describe("generateComplianceReport", () => {
    ...
    it("should generate appropriate compliance notes", async () => {
      ...
      expect(report.complianceNotes).toContain("✓ Risk assessment performed for all audit events");
    });
    ...
    it("should identify concerning patterns in compliance notes", async () => {
      ...
      expect(
        report.complianceNotes.some(note => /5 CRITICAL risk events detected/.test(note))
      ).toBe(true);
      expect(report.complianceNotes.some(note => /High deletion rate detected/.test(note))).toBe(
        true
      );
    });

    it("should handle errors in compliance report generation", async () => {
      mockAuditRepository.getAuditStatistics.mockRejectedValue(new Error("Statistics error"));

      await expect(auditService.generateComplianceReport(startDate, endDate)).rejects.toThrow(
  ...
  describe("getHighRiskEvents", () => {
    ...
    it("should handle repository errors", async () => {
      mockAuditRepository.getHighRiskEvents.mockRejectedValue(new Error("Database error"));
  ...
  describe("notifySecurityTeam", () => {
    ...
    it("should log security events without throwing", async () => {
      await expect(auditService.notifySecurityTeam(mockSecurityEvent)).resolves.not.toThrow();
    });
    ...
    it("should log CRITICAL events with error level", async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith("CRITICAL SECURITY EVENT", criticalEvent);
    });
    ...
    it("should log HIGH risk events with warn level", async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith("HIGH RISK SECURITY EVENT", highRiskEvent);
