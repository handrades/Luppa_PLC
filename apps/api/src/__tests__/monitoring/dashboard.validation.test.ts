    ...
    expectedDashboards.forEach((dashboardFile) => {
      describe(`${dashboardFile}`, () => {
        ...
        it("should have valid query targets", () => {
          ...
          panels.forEach((panel: unknown) => {
            (panel as { targets: unknown[] }).targets.forEach(
              (target: unknown) => {
        ...
        it("should use valid Prometheus queries", () => {
          ...
          panels.forEach((panel: unknown) => {
            (panel as { targets: unknown[]; title: string }).targets.forEach(
              (target: unknown) => {
                const expr = (target as { expr: string }).expr;
                const hasValidPrefix = validMetricPrefixes.some((prefix) =>
                  expr.includes(prefix),
                );
  ...
  describe("Dashboard Content Validation", () => {
    it("system-overview dashboard should have appropriate panels", () => {
      ...
      expect(
        panelTitles.some(
          (title: string) =>
            title.includes("service") || title.includes("status"),
        ),
      ).toBe(true);
      expect(
        panelTitles.some(
          (title: string) =>
            title.includes("request") || title.includes("http"),
        ),
      ).toBe(true);
    });
    ...
    it("api-performance dashboard should focus on API metrics", () => {
      ...
      expect(
        panelTitles.some(
          (title: string) =>
            title.includes("request") || title.includes("rate"),
        ),
      ).toBe(true);
      expect(
        panelTitles.some(
          (title: string) =>
            title.includes("response") || title.includes("time"),
        ),
      ).toBe(true);
      expect(panelTitles.some((title: string) => title.includes("error"))).toBe(
        true,
      );
    });
    ...
    it("database-monitoring dashboard should focus on database metrics", () => {
      ...
      expect(
        panelTitles.some((title: string) => title.includes("connection")),
      ).toBe(true);
      expect(panelTitles.some((title: string) => title.includes("pool"))).toBe(
        true,
      );
    });
    ...
    it("redis-monitoring dashboard should focus on Redis metrics", () => {
      ...
      expect(
        panelTitles.some(
          (title: string) =>
            title.includes("redis") || title.includes("memory"),
        ),
      ).toBe(true);
      expect(
        panelTitles.some(
          (title: string) => title.includes("hit") || title.includes("miss"),
        ),
      ).toBe(true);
    });
  });
