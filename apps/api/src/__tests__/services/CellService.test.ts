...
describe("CellService", () => {
  ...
  describe("createCell", () => {
    ...
    it("should validate cell name length", async () => {
      ...
      await expect(
        cellService.createCell(
          { ...validCreateInput, name: longName },
          { userId: mockUserId },
        ),
    ...
    it("should handle line number uniqueness within site", async () => {
      ...
      await expect(
        ...
        `Line number 'LINE-01' already exists in site 'Test Site'`,
      );
    });
  ...
  describe("searchCells", () => {
    ...
    it("should search cells with default parameters", async () => {
      ...
      expect(mockCellRepository.createQueryBuilder).toHaveBeenCalledWith(
        "cell",
      );
  ...
  describe("updateCell", () => {
    ...
    it("should update cell successfully", async () => {
      ...
      const result = await cellService.updateCell(
        cellId,
        updateData,
        expectedUpdatedAt,
        {
    ...
    it("should handle optimistic locking conflict", async () => {
      ...
      await expect(
        ...
        "Cell was modified by another user. Please refresh and try again.",
      );
    });
  ...
  describe("deleteCell", () => {
    ...
    it("should throw error if cell not found", async () => {
      ...
      await expect(
        cellService.deleteCell(cellId, { userId: mockUserId }),
      ).rejects.toThrow(`Cell with ID '${cellId}' not found`);
    });
  ...
  describe("validateCellUniqueness", () => {
    it("should return true for unique line number within site", async () => {
      ...
      const result = await cellService.validateCellUniqueness(
        mockSiteId,
        "LINE-01",
      );
    ...
    it("should exclude specified cell ID from check", async () => {
      ...
      await cellService.validateCellUniqueness(
        mockSiteId,
        "LINE-01",
        excludeId,
      );
  ...
  describe("validateHierarchyIntegrity", () => {
    ...
    it("should detect orphaned cells", async () => {
      ...
      expect(result.errors).toContain(
        "Found 2 orphaned cells without valid sites",
      );
      expect(result.errors).toContain(
    ...
    it("should detect duplicate line numbers within sites", async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Found 1 duplicate line numbers within sites",
      );
      expect(result.errors).toContain(
  ...
  describe("getCellSuggestions", () => {
    ...
    it("should return suggestions for valid query", async () => {
      ...
      const result = await cellService.getCellSuggestions(
        mockSiteId,
        "Cell",
        5,
      );
