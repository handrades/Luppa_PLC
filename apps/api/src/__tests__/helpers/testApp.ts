...
jest.mock("../../services/SiteService", () => {
  return {
    SiteService: jest.fn().mockImplementation(() => ({
      createSite: jest
        .fn()
        .mockImplementation(async (data: { name: string }) => {
      ...
      searchSites: jest
        .fn()
        .mockImplementation(async (filters: Record<string, unknown> = {}) => {
          ...
          if (filters.sortBy && typeof filters.sortBy === "string") {
            ...
            const sortOrder =
              String(filters.sortOrder || "").toUpperCase() === "DESC" ? -1 : 1;
            sites.sort(
      ...
      getSiteStatistics: jest.fn().mockImplementation(async () => ({
        ...
        totalEquipment: 0,
        averageCellsPerSite:
          testData.sites.size > 0
            ? testData.cells.size / testData.sites.size
            : 0,
        averageEquipmentPerSite: 0,
        sitesWithoutCells: 0,
        sitesWithoutEquipment: testData.sites.size, // Assuming no equipment in test data
      })),
      getSiteSuggestions: jest
        .fn()
        .mockImplementation(async (query?: string, limit = 10) => {
          let sites = Array.from(testData.sites.values());
        ...
        .mockImplementation(async (name: string, excludeId?: string) => {
          ...
          const duplicate = existingSites.find(
            (site: Record<string, unknown>) =>
              site.name === name && site.id !== excludeId,
          );
          return !duplicate;
        }),
      updateSite: jest
            ...
            if (data.name && data.name !== site.name) {
              ...
              const duplicate = existingSites.find(
                (s: Record<string, unknown>) =>
                  s.name === data.name && s.id !== id,
              );
              if (duplicate) {
                const error = new Error(
                  `Site name '${data.name}' already exists`,
                );
                error.name = "ConflictError";
                throw error;
              }
    ...
    CellService: jest.fn().mockImplementation(() => ({
          ...
          async (data: {
            ...
            const duplicate = existingCells.find(
              (cell: Record<string, unknown>) =>
                cell.siteId === data.siteId &&
                cell.lineNumber === data.lineNumber.toUpperCase(),
            );
            if (duplicate) {
              const error = new Error(
                `Line number '${data.lineNumber}' already exists in this site`,
              );
              error.name = "ConflictError";
              throw error;
            }
      ...
      searchCells: jest
        ...
        .mockImplementation(async (filters: Record<string, unknown> = {}) => {
          ...
          if (filters.siteId) {
            cells = cells.filter(
              (cell: Record<string, unknown>) => cell.siteId === filters.siteId,
            );
          }
      ...
      getCellSuggestions: jest
        .fn()
        .mockImplementation(
          async (siteId?: string, query?: string, limit = 10) => {
            let cells = Array.from(testData.cells.values());
      ...
      validateCellUniqueness: jest
        .fn()
        .mockImplementation(
          async (siteId: string, lineNumber: string, excludeId?: string) => {
            const existingCells = Array.from(testData.cells.values());
            const duplicate = existingCells.find(
              (cell: Record<string, unknown>) =>
                cell.siteId === siteId &&
