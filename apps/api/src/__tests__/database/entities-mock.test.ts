...
describe("Database Entities Validation (Mock)", () => {
  ...
  describe("Entity Metadata Validation", () => {
    ...
    it("should have correct table names", () => {
      ...
      const equipmentMetadata = entityMetadatas.find(
        (e) => e.name === "Equipment",
      );
      ...
      const notificationMetadata = entityMetadatas.find(
        (e) => e.name === "Notification",
      );
    ...
    it("should have proper timestamp columns", () => {
      ...
      baseEntityNames.forEach((entityName) => {
        ...
        const createdAtColumn =
          metadata!.findColumnWithPropertyName("createdAt");
        expect(createdAtColumn).toBeDefined();
        expect(createdAtColumn!.type).toContain("timestamp");

        const updatedAtColumn =
          metadata!.findColumnWithPropertyName("updatedAt");
  ...
  describe("Entity Relationships", () => {
    it("should have correct foreign key relationships", () => {
      ...
      expect(plcEquipmentRelation!.inverseEntityMetadata.name).toBe(
        "Equipment",
      );

      // Tag -> PLC relationship
      const tagMetadata = entityMetadatas.find((e) => e.name === "Tag");
      const tagPlcRelation = tagMetadata!.manyToOneRelations.find(
        (r) => r.propertyName === "plc",
      );
    ...
    it("should have correct inverse relationships", () => {
      ...
      const plcMetadata = entityMetadatas.find((e) => e.name === "PLC");
      const plcTagsRelation = plcMetadata!.oneToManyRelations.find(
  ...
  describe("Column Types and Constraints", () => {
    it("should have correct PostgreSQL-specific column types", () => {
      ...
      const auditMetadata = entityMetadatas.find((e) => e.name === "AuditLog");
      const oldValuesColumn =
        auditMetadata!.findColumnWithPropertyName("oldValues");
      expect(oldValuesColumn!.type).toBe("jsonb");

      const newValuesColumn =
        auditMetadata!.findColumnWithPropertyName("newValues");
      expect(newValuesColumn!.type).toBe("jsonb");

      // AuditLog should have text array column
      const changedFieldsColumn =
        auditMetadata!.findColumnWithPropertyName("changedFields");
      ...
      const ipAddressColumn =
        plcMetadata!.findColumnWithPropertyName("ipAddress");
    ...
    it("should have proper enum column types", () => {
      ...
      const equipmentTypeColumn =
        equipmentMetadata!.findColumnWithPropertyName("equipmentType");
      ...
      const dataTypeColumn =
        tagMetadata!.findColumnWithPropertyName("dataType");
      ...
      const riskLevelColumn =
        auditMetadata!.findColumnWithPropertyName("riskLevel");
    ...
    it("should have proper nullable constraints", () => {
      ...
      const firstNameColumn =
        userMetadata!.findColumnWithPropertyName("firstName");
      expect(firstNameColumn!.isNullable).toBe(false);

      // Optional fields should be nullable
      const plcMetadata = entityMetadatas.find((e) => e.name === "PLC");
      const firmwareVersionColumn =
        plcMetadata!.findColumnWithPropertyName("firmwareVersion");
  ...
  describe("TypeORM Configuration Validation", () => {
    ...
    it("should include all entity classes", () => {
      const entities = mockDataSource.options.entities as Array<
        new () => object
      >;
