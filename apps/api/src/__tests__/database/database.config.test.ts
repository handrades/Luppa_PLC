...
jest.mock("../../config/database", () => {
  const getDatabaseConfig = () => {
    ...
    const password =
      process.env.DB_PASSWORD || process.env.TEST_DB_PASSWORD || "test-db-pass";

    // Validation logic
    if (isNaN(port) || port < 1 || port > 65535) {
      throw new Error(
        "Invalid DB_PORT value. Must be a number between 1 and 65535.",
      );
    }
    ...
    if (poolMin < 1) {
      throw new Error(
        "Invalid pool settings: minimum connections must be at least 1",
      );
    }

    if (poolMax < poolMin) {
      throw new Error(
        "Invalid pool settings: maximum connections must be greater than minimum",
      );
    }
