...
describe("Equipment Routes Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    const mockEquipmentRepository = {
      ...
      save: jest
        .fn()
        .mockImplementation((equipment) => Promise.resolve({ ...equipment })),
      findOne: jest.fn(),
      find: jest.fn(),
  ...
  beforeEach(async () => {
    ...
    const mockEquipmentService = EquipmentService as jest.MockedClass<
      typeof EquipmentService
    >;
    const mockServiceInstance =
      new mockEquipmentService() as jest.Mocked<EquipmentService>;

    // Track deleted equipment IDs
    const deletedEquipmentIds = new Set<string>();

    // Configure createEquipment mock
    mockServiceInstance.createEquipment.mockImplementation(
      async (equipmentData) => {
        // Check for duplicate tag ID scenario
        if (
          equipmentData.plcData.tagId === "PRESS_001" &&
          equipmentData.name === "Another Press"
        ) {
    ...
    mockServiceInstance.getEquipmentById.mockImplementation(
      async (id: string) => {
    ...
    mockServiceInstance.deleteEquipment.mockImplementation(
      async (id: string) => {
  ...
  describe("PUT /api/v1/equipment/:id", () => {
    it("should update equipment successfully", async () => {
      ...
      expect(response.body.equipment.plcs[0].description).toBe(
        "Updated description",
      );
  ...
  describe("POST /api/v1/equipment/bulk", () => {
    it("should perform bulk delete operation", async () => {
      ...
      expect(response.body.message).toContain(
        "Successfully deleted 2 equipment items",
      );
    });
