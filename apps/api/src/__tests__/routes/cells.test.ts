...
describe("Cells Routes", () => {
  ...
  describe("POST /cells", () => {
    ...
    it("should validate required fields", async () => {
      ...
      expect(response.body.error.details.siteId).toContain(
        "Site ID is required",
      );
    ...
    it("should validate cell name length", async () => {
      ...
      expect(response.body.error.message).toContain("Validation failed");
      expect(response.body.error.details.name).toContain(
        "Cell name cannot exceed 100 characters",
      );
    });
    ...
    it("should handle line number conflicts within site", async () => {
      ...
      expect(response.body.message).toContain(
        `Line number 'CONFLICT-01' already exists`,
      );
    });
  ...
  describe("GET /cells", () => {
    beforeEach(async () => {
      ...
      await request(app)
        .post("/api/v1/cells")
        .set("Authorization", `Bearer ${authToken}`)
        .send({ ...testCellData, name: `Cell A ${timestamp}`, lineNumber: `A${timestamp}` });

      await request(app)
        .post("/api/v1/cells")
        .set("Authorization", `Bearer ${authToken}`)
        .send({ ...testCellData, name: `Cell B ${timestamp}`, lineNumber: `B${timestamp}` });
    });
    ...
    it("should apply site filter", async () => {
      ...
      expect(
        response.body.data.every(
          (cell: { siteId: string }) => cell.siteId === testSiteId,
        ),
      ).toBe(true);
    });
  ...
  describe("PUT /cells/:id", () => {
    ...
    it("should validate updated line number uniqueness", async () => {
      ...
      expect(response.body.message).toContain(
        "Line number 'ANOTHER-01' already exists",
      );
    });
  ...
  describe("POST /cells/bulk", () => {
    ...
    it("should handle bulk move operation (not implemented)", async () => {
      ...
      expect(response.body.message).toContain(
        "Bulk move operation is not yet implemented",
      );
    });
    ...
    it("should validate cell IDs array", async () => {
      ...
      expect(response.body.message).toContain(
        "At least one cell ID is required",
      );
    });
