...
describe("UserService", () => {
  ...
  beforeEach(() => {
    ...
    mockEmailService = {
      ...
      sendAccountDeactivationNotification: jest
        .fn()
    ...
    (PasswordResetService as jest.Mock).mockImplementation(
      () => mockPasswordResetService,
    );
    ...
    (AppDataSource.manager.getRepository as jest.Mock).mockImplementation(
      (entity) => {
        if (entity === Role) {
  ...
  describe("updateUser", () => {
    ...
    it("should send role assignment notification when role changes", async () => {
      ...
      expect(
        mockEmailService.sendRoleAssignmentNotification,
  ...
  describe("softDeleteUser", () => {
    it("should soft delete user successfully", async () => {
      ...
      expect(
        mockEmailService.sendAccountDeactivationNotification,
      ).toHaveBeenCalledWith(mockUser, "admin@example.com");
    });

    it("should throw error if user not found", async () => {
      mockUserRepository.findWithRole.mockResolvedValue(null);

      await expect(userService.softDeleteUser("nonexistent")).rejects.toThrow(
        "User not found",
      );
    });
  ...
  describe("assignRole", () => {
    ...
    it("should throw error if user not found", async () => {
      ...
      await expect(
        userService.assignRole("nonexistent", "role-789", "admin"),
      ).rejects.toThrow("User not found");
    });
    ...
    it("should throw error if role not found", async () => {
      ...
      await expect(
        userService.assignRole("user-123", "nonexistent", "admin"),
    ...
    it("should throw error if user already has the role", async () => {
      ...
      await expect(
        userService.assignRole("user-123", "role-456", "admin"),
  ...
  describe("searchUsers", () => {
    it("should return paginated user results", async () => {
      ...
      mockUserRepository.searchUsersWithPagination.mockResolvedValue(
        mockResult,
      );

      const result = await userService.searchUsers(filters);

      expect(mockUserRepository.searchUsersWithPagination).toHaveBeenCalledWith(
        filters,
      );
  ...
  describe("password reset functionality", () => {
    it("should generate password reset token", async () => {
      ...
      mockPasswordResetService.generatePasswordResetToken.mockResolvedValue(
        token,
      );

      const result =
        await userService.generatePasswordResetToken("test@example.com");

      expect(
        mockPasswordResetService.generatePasswordResetToken,
  ...
  describe("isEmailAvailable", () => {
    ...
    it("should exclude specific user ID when checking availability", async () => {
      ...
      const result = await userService.isEmailAvailable(
        "test@example.com",
        "user-123",
      );
