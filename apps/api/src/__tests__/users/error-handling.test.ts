...
jest.mock("../../validation/userSchemas", () => ({
  ...
  validateSchema: jest.fn(() => (data) => {
    ...
    if (
      ...
      if (typeof converted.page === "string")
        converted.page = parseInt(converted.page, 10);
...
describe("User Management Error Handling", () => {
  ...
  beforeEach(() => {
    ...
    (
      PasswordResetService as jest.MockedClass<typeof PasswordResetService>
    ).mockImplementation(
      () => mockPasswordResetService as jest.Mocked<PasswordResetService>,
    );
    (
      EmailNotificationService as jest.MockedClass<
        typeof EmailNotificationService
      >
    ).mockImplementation(
      () => mockEmailService as jest.Mocked<EmailNotificationService>,
    );
  ...
  describe("Business Logic Error Handling", () => {
    describe("User Creation Business Errors", () => {
      ...
      it("should return 400 for default role not found", async () => {
        mockUserService.createUser.mockRejectedValue(
          new Error("Default Engineer role not found"),
        );
    ...
    describe("User Deletion Business Errors", () => {
      it("should return 404 for deleting non-existent user", async () => {
        mockUserService.softDeleteUser.mockRejectedValue(
          new Error("User not found"),
        );
      ...
      it("should return 400 for already inactive user", async () => {
        mockUserService.softDeleteUser.mockRejectedValue(
          new Error("User is already inactive"),
        );
    ...
    describe("Role Assignment Business Errors", () => {
      it("should return 404 for assigning role to non-existent user", async () => {
        mockUserService.assignRole.mockRejectedValue(
          new Error("User not found"),
        );
  ...
  describe("System Error Handling", () => {
    ...
    describe("Memory and Resource Errors", () => {
      it("should handle out of memory errors", async () => {
        mockUserService.getUserStats.mockRejectedValue(
  ...
  describe("Error Response Consistency", () => {
    ...
    it("should not leak sensitive information in error messages", async () => {
      mockUserService.createUser.mockRejectedValue(
        new Error(
          "Connection string: postgres://user:password@host:5432/db failed",
        ),
      );
  ...
  describe("HTTP Status Code Validation", () => {
    ...
    statusCodeTests.forEach(({ operation, method, path, successStatus }) => {
      ...
      testFn(
        `should return ${successStatus} for successful ${operation}`,
        async () => {
          ...
          const requestBuilder =
            request(app)[
              method.toLowerCase() as keyof typeof request.prototype
            ](path);
