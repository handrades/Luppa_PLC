...
describe("Password Reset Flow", () => {
  ...
  describe("POST /auth/password-reset", () => {
    ...
    it("should return success even for non-existent email (security)", async () => {
      ...
      expect(
        mockPasswordResetService.generatePasswordResetToken,
      ).toHaveBeenCalledWith(nonExistentEmail);
      expect(
        mockEmailService.sendPasswordResetNotification,
    ...
    it("should include reset URL with correct frontend URL", async () => {
      ...
      expect(
        mockPasswordResetService.generatePasswordResetToken,
      ).toHaveBeenCalledWith(validResetRequest.email);
    });
  });
  ...
  describe("Password Reset Token Security", () => {
    it("should generate cryptographically secure tokens", async () => {
      mockPasswordResetService.generatePasswordResetToken.mockImplementation(
        async () => {
          // Simulate secure token generation
          return (
            "crypto-secure-token-" + Math.random().toString(36).substring(2)
          );
        },
      );
