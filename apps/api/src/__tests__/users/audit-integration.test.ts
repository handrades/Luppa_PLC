...
describe("User Management Audit Integration", () => {
  ...
  beforeEach(() => {
    ...
    (auditContextMiddleware as jest.Mock).mockImplementation(
      async (req, _res, next) => {
        ...
        const sessionId =
          (req.headers["x-session-id"] as string) || req.sessionID || "";

        // Simulate the SQL context setting calls that tests expect
        await queryRunner.connect();
        await queryRunner.query("SET app.current_user_id = $1", [
          TEST_JWT.userId,
        ]);
  ...
  describe("Compliance and Reporting", () => {
    it("should track user activity for compliance reporting", async () => {
      ...
      const userActivity = await mockRepository.getUserActivitySummary(
        TEST_JWT.userId,
        30,
      );
