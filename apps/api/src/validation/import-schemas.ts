...
export const exportFiltersSchema = Joi.object({
  ...
  cellTypes: Joi.array()
    .items(Joi.string().valid('production', 'warehouse', 'testing', 'packaging'))
    .optional(),
  ...
  ipRange: Joi.string()
    ...
    .messages({
      'string.ip':
        'IP range must be in CIDR notation (e.g., 192.168.1.0/24 for IPv4 or 2001:db8::/32 for IPv6)',
    }),
...
export const importHistoryQuerySchema = Joi.object({
  ...
  endDate: Joi.date()
    // biome-ignore lint/suspicious/noThenProperty: Joi's .when() requires 'then' property
    .when('startDate', {
      is: Joi.exist(),
