...
export class ImportExportService {
      ...
      parser.on("data", (row: string[]) => {
        if (isFirstRow) {
          ...
          if (unknownHeaders.length > 0) {
            result.headerErrors.push(
              `Unknown headers: ${unknownHeaders.join(", ")}`,
            );
          }
        } else {
          records.push(row);
  ...
  async importPLCs(
    ...
    if (options.validateOnly) {
      return {
        success: true,
        importId,
        totalRows: validation.preview.length,
        successfulRows: validation.preview.length - validation.rowErrors.length,
  ...
  private async processRow(
    ...
    if (!equipment) {
      if (options.createMissing) {
        ...
        throw new Error(
          `Equipment '${row.equipment_name}' not found in cell '${row.cell_name}'`,
        );
      }
    }
  ...
  async exportPLCs(
    ...
    const csvData = plcs.map((plc) => ({
      ...
      line_number: plc.equipment?.cell?.lineNumber || "",
    ...
    return new Promise((resolve, reject) => {
      const records = [
        ALL_HEADERS,
        ...csvData.map((row) =>
          ALL_HEADERS.map((header) => (row as Record<string, string>)[header]),
        ),
      ];
