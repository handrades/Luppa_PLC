...
const plcDetailsSchema = z.object({
  tagId: z
    ...
    .regex(/^[a-zA-Z0-9_]+$/, 'Tag ID must be alphanumeric with underscores only'),
...
const networkConfigSchema = z.object({
  ...
  firmwareVersion: z.string().max(50, 'Firmware version must be less than 50 characters').optional().or(z.literal('')),
});
...
export const equipmentUpdateSchema = equipmentCreateSchema.extend({
  updatedAt: z.string().min(1, 'Updated timestamp is required for updates').datetime('Invalid timestamp format'),
});
```
