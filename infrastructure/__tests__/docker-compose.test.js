...
describe("Docker Compose Configuration Tests", () => {
  ...
  beforeAll(() => {
    ...
    const prodComposePath = path.join(
      __dirname,
      "../../docker-compose.prod.yml",
    );
    const swarmComposePath = path.join(
      __dirname,
    ...
    try {
      ...
      throw new Error(
        `Failed to load production compose file: ${error.message}`,
      );
    }
...
describe("Infrastructure Files Validation", () => {
  ...
  test("Prometheus configuration should be valid YAML", () => {
    const prometheusConfigPath = path.join(
      __dirname,
  ...
  test("Grafana configuration should exist", () => {
    const grafanaConfigPath = path.join(
      __dirname,
