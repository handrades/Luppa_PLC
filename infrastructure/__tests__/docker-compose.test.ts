...
describe("Docker Compose Configuration", () => {
  ...
  describe("PostgreSQL Service", () => {
    ...
    test("should have persistent volume mounted", () => {
      expect(postgresService.volumes).toContain(
  ...
  describe("API Service", () => {
    ...
    test("should depend on database services", () => {
      ...
      expect(
        (apiService.depends_on as Record<string, { condition: string }>)
          ?.postgres?.condition,
      ).toBe("service_healthy");
      expect(
        (apiService.depends_on as Record<string, { condition: string }>)?.redis
          ?.condition,
      ).toBe("service_healthy");
    });
    ...
    test("should have volume mounts for development", () => {
      ...
      expect(apiService.volumes).toContain(
  ...
  describe("Web Service", () => {
    ...
    test("should have volume mounts for development", () => {
      // Check for selective source code mounting
      expect(webService.volumes).toContain(
...
describe("Supporting Files", () => {
  ...
  test("API Dockerfile.dev should exist", () => {
    const apiDockerfilePath = path.resolve(
      __dirname,
  ...
  test("Web Dockerfile.dev should exist", () => {
    const webDockerfilePath = path.resolve(
      __dirname,
