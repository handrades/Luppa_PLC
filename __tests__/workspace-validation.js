...
test("package.json has correct workspace scripts", () => {
  ...
  expect(content.scripts.test).toBe(
    "jest --config config/jest.config.js --passWithNoTests",
  );
...
test("tsconfig.json has workspace path mappings", () => {
  const content = JSON.parse(
    readFileSync(join(rootDir, "config/tsconfig.json"), "utf-8"),
  );
  expect(content.compilerOptions.paths["@shared-types/*"][0]).toBe(
    "packages/shared-types/src/*",
  );
  expect(content.compilerOptions.paths["@ui-components/*"][0]).toBe(
    "packages/ui-components/src/*",
  );
  expect(content.compilerOptions.paths["@config/*"][0]).toBe(
    "packages/config/src/*",
  );
});
